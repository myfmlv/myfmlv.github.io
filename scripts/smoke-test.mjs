...
expectIncludes(html, 'id="adminDataStatusToggle"', 'index.html must include the admin data status toggle')
expectIncludes(html, 'id="adminDataStatusPanel"', 'index.html must include the admin data status panel')
...
expectIncludes(html, 'id="dataStatusDebug"', 'index.html must include the data status debug target')
expectIncludes(html, '데이터 상태 초기화 중', 'index.html must use a short neutral initial data status message')
expect(!html.includes('데이터 상태를 확인하는 중입니다'), 'index.html must not include the old persistent data status loading text')
expect(!html.includes('기준일과 갱신시각을 불러오는 중입니다'), 'index.html must not include the old persistent status metadata loading text')
...
expectIncludes(app, 'function initAdminDataStatusToggle(', 'app.js must initialize the admin data status toggle')
expectIncludes(app, 'function withCacheBust(', 'app.js must cache-bust data fetches')
...
expectIncludes(app, 'createInitialDataStatus', 'app.js must use the shared data status model')
expectIncludes(app, 'finalizeDataStatus', 'app.js must finalize data status in terminal paths')
...
expectIncludes(css, '.admin-data-status-toggle', 'styles.css must style the admin data status toggle')
expectIncludes(css, '.data-status-panel[hidden]', 'styles.css must hide the data status panel by default')
expectIncludes(css, ".data-status-panel[data-status='stale']", 'styles.css must style stale data status')
expectIncludes(css, ".data-status-panel[data-status='partial']", 'styles.css must style partial data status')
...
expect(fs.existsSync(path.join(root, 'src/utils/dataStatus.js')), 'src/utils/dataStatus.js must exist')
expect(fs.existsSync(path.join(root, 'data/update-status.json')), 'data/update-status.json must exist')
expect(fs.existsSync(path.join(root, '.github/workflows/update-market-data.yml')), 'update-market-data workflow must exist')
